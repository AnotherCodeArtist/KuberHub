...
if [ $(id -u) == 0 ] ; then
    ...
    exec sudo -E -H -u $JUPYTERHUB_USER PATH=$PATH PYTHONPATH=$PYTHONPATH stack exec --allow-different-user --cwd /home/jovyan/work /opt/conda/bin/jupyterhub-singleuser -- --ip=0.0.0.0 --port=8888 --allow-root
    #stack exec --allow-different-user --cwd /home/jovyan/work $1 -- ${@:2}
    #exec su $JUPYTERHUB_USER -c 'env PATH="$PATH:/opt/conda/bin" stack exec "$1" -- "${@:2}"' -- "$@"
else
    # Exec the command
