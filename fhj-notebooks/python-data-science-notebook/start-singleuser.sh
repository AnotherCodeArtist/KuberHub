...
echo "In start-singleuser.sh with arguments $@"

# set default ip to 0.0.0.0
if [[ "$NOTEBOOK_ARGS $@" != *"--ip="* ]]; then
  NOTEBOOK_ARGS="--ip=0.0.0.0 $NOTEBOOK_ARGS"
  echo "Added --ip config: $NOTEBOOK_ARGS"
fi
...
if [ ! -z "$JPY_USER" ]; then
  ...
  echo "Adding JPY_USER: $NOTEBOOK_ARGS"
fi
...
echo "Calling start.sh with: $NOTEBOOK_BIN $NOTEBOOK_ARGS $@"
